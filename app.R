...
ui <- fluidPage(
  ...
  helpText(h5(
    "Enter the average hourly bus operator wage for your agency.", br(),
    "If nothing is entered, this value will default to $15/hr"
  ), style = "color:black"),
  textInput(
    inputId = "wage.input",
  ...
  helpText(h5(
    "Enter the per mile operating cost for your agency.", br(),
    "If nothing is entered, this value will default to $9/mi"
  ), style = "color:black"),
  textInput(
    inputId = "op_cost.input",
  ...
  helpText(h5(
    "Upload the data on your existing depots. The file should have at least the following columns:", br(),
  ...
  helpText(h5(
    "Upload the data on the vacant properties you're considering. The file should have at least the following columns:", br(),
...
server <- function(input, output) {
  observeEvent(input$submit, { # only run when user clicks submit
    ...
    if (is.null(input$vac.input)) {
      return(NULL)
    }
    if (is.null(input$garages.input)) {
      return(NULL)
    }
    if (is.null(input$cal.input)) {
      return(NULL)
    }
    if (is.null(input$trips.input)) {
      return(NULL)
    }
    if (is.null(input$routes.input)) {
      return(NULL)
    }
    if (is.null(input$times.input)) {
      return(NULL)
    }
    if (is.null(input$stops.input)) {
      return(NULL)
    }
    ...
    trip_first_last <- gtfs %>%
      group_by(route_id, trip_id) %>%
      summarize(
        first_stop = stop_id[which.min(stop_sequence)],
    ...
    deadhead <- trip_times %>%
      group_by(block_id) %>%
      summarize(
        first_trip = min(arrival_time[which(arrival_time > "03:00:00")]),
    ...
    if (length(midday_dh_ids) != 0) {
      deadhead2 <- trip_times[trip_times$block_id %in% midday_dh_ids, ] %>%
        group_by(block_id) %>%
        summarize(
          first_trip = min(arrival_time[which(arrival_time > "15:00:00")]), # second outbound deadhead trip
    ...
    final_match_data <- cbind(
      dh_comp[, 1:10], dh_garages_round$depot_match, dh_garages_round$min, dh_dist_garages_round$match_dist,
      winner_data$final_matches
    )
    colnames(final_match_data)[1:14] <- c(
      "block_id", "first_trip_time", "last_trip_time",
    ...
    output$dh_savings.output <- renderText(paste0(
      "This would save ~", dh_savings_time, " hours or ", dh_savings_dist,
      " miles of deadhead travel per weekday."
    ))

    output$dh_cost_savings.output <- renderText(paste0(
      "This translates to saving about $", wage_savings, " in wage costs and $",
      op_cost_savings, " in operating costs per weekday."
    ))

    output$downloadData <- downloadHandler(
      filename = "Final Bus_Depot Matches.csv",
    ...
    output$downloadData2 <- downloadHandler(
      filename = "Properties Considered.csv",
      content = function(file) {
